...
public class PatternFactory {
    ...
    public static Pattern getPattern(JsonParser p, String type, EventImpl pEvent, EventImpl sEvent,
                                     ChainEvents chainEvents, ProbabilityBound probabilityBound, TimeBound timeBound,
                                     EventConstraint constrainEvent, int upperLimit, int frequency, String timeUnit) throws JsonMappingException {
        // Occurrence
        ...
        } else if ("RelaxedPrecedenceChain1N".equalsIgnoreCase(type)) {
            return new RelaxedPrecedenceChain1N(pEvent, sEvent, chainEvents, timeBound, constrainEvent, probabilityBound);
        } else if ("RelaxedPrecedenceChainN1".equalsIgnoreCase(type)) {
            return new RelaxedPrecedenceChainN1(pEvent, sEvent, chainEvents, timeBound, constrainEvent, probabilityBound);
        ...
        } else if ("RelaxedResponseChain1N".equalsIgnoreCase(type)) {
            return new RelaxedResponseChain1N(pEvent, sEvent, chainEvents, timeBound, constrainEvent, probabilityBound);
        } else if ("RelaxedResponseChainN1".equalsIgnoreCase(type)) {
            return new RelaxedResponseChainN1(pEvent, sEvent, chainEvents, timeBound, constrainEvent, probabilityBound);
